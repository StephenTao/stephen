...
from highlander.openstack.common import log
from highlander import version


launch_opt = cfg.ListOpt(
    'server',
    default=['all'],
    help='Specifies which highlander server to start by the launch script. '
         'Valid options are all or any combination of '
         'api, engine, and executor.'
)

api_opts = [
    cfg.StrOpt('host', default='0.0.0.0', help='Highlander API server host'),
    cfg.IntOpt('port', default=8989, help='Highlander API server port')
]

pecan_opts = [
    cfg.StrOpt('root', default='highlander.api.controllers.root.RootController',
               help='Pecan root controller'),
    cfg.ListOpt('modules', default=["highlander.api"],
...
CONF.import_opt('verbose', 'highlander.openstack.common.log')
CONF.set_default('verbose', True)
CONF.import_opt('debug', 'highlander.openstack.common.log')
CONF.import_opt('log_dir', 'highlander.openstack.common.log')
CONF.import_opt('log_file', 'highlander.openstack.common.log')
CONF.import_opt('log_config_append', 'highlander.openstack.common.log')
CONF.import_opt('log_format', 'highlander.openstack.common.log')
CONF.import_opt('log_date_format', 'highlander.openstack.common.log')
CONF.import_opt('use_syslog', 'highlander.openstack.common.log')
CONF.import_opt('syslog_log_facility', 'highlander.openstack.common.log')

# Extend oslo default_log_levels to include some that are useful for highlander
