...
function usage {
  echo "Usage: $0 [OPTION]..."
  echo "Run Highlander's test suite(s)"
  echo ""
...
function run_tests {
  ...
  if [ $debug -eq 1 ]; then
    if [ "$testropts" = "" ] && [ "$testrargs" = "" ]; then
      ...
      # provided.
      testrargs="discover ./highlander/tests/unit"
    fi
  ...
  OS_TEST_PATH=$(echo $testrargs|grep -o 'highlander\.tests[^[:space:]:]*\+'|tr . /)
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='highlander/*' --omit='highlander/openstack/common/*' -d covhtml -i
  fi
...
TESTRTESTS="python -m highlander.openstack.common.lockutils python setup.py testr"
